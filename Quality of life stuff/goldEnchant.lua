...
local T = {
    ...
    [15] = {
        ...
        {"Enchant Weapon|r - Mongoose", 2673, false},
		...
		for k, v in pairs(T[ID]) do
			if v[2] == intid then
				...
				if item then
					...
					if v[3] then
						local sql = "SELECT * FROM item_template WHERE entry = "..player:GetItemByEntry(item)
						...
						if pVar[player:GetName()] == 151 then
							if (WType == 1 or WType == 5 or WType == 6 or WType == 8 or WType == 10) then
								...
								player:SendAreaTriggerMessage("|Cffff0000[Enchanter]|r: You do not have a Two-Handed Weapon equipped!")
							end
						elseif pVar[player:GetName()] == 161 then
							if (Wtype == 6) then
								...
								player:SendAreaTriggerMessage("|Cffff0000[Enchanter]|r: You do not have a Shield equipped!")
							end
						end
					else
						item:ClearEnchantment(0)
					...
					player:SendAreaTriggerMessage("|Cffff0000[Enchanter]|r: You have no item to enchant in the selected slot!")
				end
			end
		end
