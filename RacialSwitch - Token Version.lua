...
local TokenId = 29434 -- ItemId
local TokenCount = 1 -- How many X items it cost
...
local T = {
    ...
    [10] = {"Draenei", 10, {59542, 28875, 6562, 59221}}
 -- Draenei
}
...
local function OnHelloRacialSwitch(event, player, creature)
    if player:IsInCombat() then
        ...
        for i, v in ipairs(T) do
            player:GossipMenuAddItem(3, v[1], i, 0, nil, "Are You Sure?")
        end
        player:GossipSendMenu(1, creature)
    end
end

--This will unlearn all the racials from the char once the person select other one. Could probably been done better. Just that some classes got duplicated Spells but with X other IDS
local unlearn = {
    54562,
    21563,
    20576,
...
local function OnGossipRacialSwitch(event, player, creature, sender, intid, code, menu_id)
    ...
    if (intid == 0) then
        for i, v in ipairs(T[sender]) do
            if (i > 2) then
                if player:HasItem(TokenId, TokenCount, true) then
                    player:RemoveItem(TokenId, TokenCount)
                    ...
                    if player:HasSpell(28877) then
                        ...
                        player:SendBroadcastMessage(
                            "|cFFFFFF9F" ..
                                creature:GetName() ..
                                    " Says: I Took " .. TokenCount .. " " .. GetItemLink(TokenId) .. " From You."
                        )
                        return
                    end
                else
                    player:SendBroadcastMessage(
                        "|cFFFFFF9F" ..
                            creature:GetName() ..
                                " Says: You Require " .. TokenCount .. " " .. GetItemLink(TokenId) .. ""
                    )
                    return
                end
            end
